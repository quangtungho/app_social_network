...
import vn.techres.line.databinding.FragmentOrderCustomerComFirmedBinding
...
class OrderCustomerComFirmedFragment : BaseBindingFragment<FragmentOrderCustomerComFirmedBinding>(FragmentOrderCustomerComFirmedBinding::inflate), ChooseOrderCustomerHandler {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = OrderCustomerAdapter(mainActivity!!.baseContext)
        binding.rcOrder.layoutManager  = LinearLayoutManager(context, RecyclerView.VERTICAL, false)
    ...
    private fun getOrderCustomer(page : Int) {
        ...
        baseRequest.request_url = "/api/customers/" +
                user.id +
                "/orders?order_statuses=" +
                resources.getString(R.string.orderCustomerConFirm) +
                "&page=$page" +
                "&restaurant_id=" +
                ...
                .subscribe(object : Observer<OrderCustomerResponse> {
                    ...
                    override fun onNext(response: OrderCustomerResponse) {
                        if (response.status == AppConfig.SUCCESS_CODE) {
                            ...
                            checkOrders(dataOrderCustomer.list)
                            if(data.size == 0){
