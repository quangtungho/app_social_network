...
import vn.techres.line.adapter.account.ProfileVideoAdapter
...
import vn.techres.line.databinding.FragmentProfileVideoBinding
...
class ProfileVideoFragment : BaseBindingFragment<FragmentProfileVideoBinding>(FragmentProfileVideoBinding::inflate), ClickMediaProfile {
    private val profileActivity: ProfileActivity?
        get() = activity as ProfileActivity?

    private var profileVideoAdapter: ProfileVideoAdapter? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        profileVideoAdapter = ProfileVideoAdapter(requireActivity().baseContext)
        binding.recyclerView.adapter = profileVideoAdapter

        profileVideoAdapter?.setClickMediaProfile(this)
        arguments?.let {
            idProfile = it.getInt(TechresEnum.ID_USER.toString())
        }

        getVideo(idProfile, page)
    ...
    private fun getVideo(userId: Int, page: Int) {
        val baseRequest = BaseParams()
        baseRequest.http_method = 0
        baseRequest.request_url =
            "/api/customer-media-album-contents?media_type=0&customer_id=$userId&customer_media_album_id=-1&page=$page&limit=$limit"
            ...
            .subscribe(object : Observer<MediaProfileResponse> {
                ...
                override fun onNext(response: MediaProfileResponse) {
                    ...
                    if (response.status == AppConfig.SUCCESS_CODE) {
                        ...
                        profileVideoAdapter!!.setDataSource(data)
    ...
    override fun onClickMedia(url: ArrayList<MediaProfile>, position: Int) {
        ...
        val intent = Intent(profileActivity, MediaSliderActivity::class.java)
