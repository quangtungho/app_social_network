package vn.techres.line.fragment.bill
...
import vn.techres.line.activity.VATBillActivity
...
import vn.techres.line.databinding.FragmentBillBinding
...
open class BillFragment : BaseBindingFragment<FragmentBillBinding>(FragmentBillBinding::inflate),
    BillHandler {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapterBill = BillAdapter(requireActivity())
        ...
        if (bundle != null) {
            idOrder = bundle.getInt(TechresEnum.KEY_ID_ORDER_CUSTOMER.toString()).toString().toInt()
    ...
    private fun setListener() {
        ...
        binding.lnOrderPoint.setOnClickListener {
            ...
            cacheManager.put(
                ...
                requireActivity().resources.getString(R.string.bill_fragment)
            )
        ...
        binding.footer.lnVAT.setOnClickListener {
            val intent = Intent(mainActivity, VATBillActivity::class.java)
            intent.putExtra(TechresEnum.ORDER_ID.toString(), idOrder)
            intent.putExtra(TechresEnum.FOOD_VAT_AMOUNT.toString(), String.format(
                "%s %s",
    ...
    private fun dialogQuestionBill(typePoint: String, typeUsing: String) {
        ...
        if (typeUsing == "ok") {
            edNoteBill.text = requireActivity().resources.getString(R.string.using_point_notification)
            btnUsingPointBill.text = requireActivity().resources.getString(R.string.using)
        } else {
            edNoteBill.text = requireActivity().resources.getString(R.string.cancel_using_point_notification)
            btnUsingPointBill.text = requireActivity().resources.getString(R.string.cancel_using_point)
        }
            ...
            when (typePoint) {
                ...
                TechresEnum.ACCUMULATE.toString() -> {
                    if ((membershipPointUsed + membershipPromotionPointUsed) > 0) {
                        val differencePoint = maximumAccumulatePointAllowUseInEachBill - (membershipPointUsed + membershipPromotionPointUsed)
                ...
                TechresEnum.PROMOTION.toString() -> {
                    if((membershipPointUsed + membershipAccumulatePointUsed) > 0){
                        val differencePromotion = maximumAccumulatePointAllowUseInEachBill - (membershipPointUsed + membershipAccumulatePointUsed)
                        if (differencePromotion <= promotionPoint) {
                            membershipPromotionPointUsed = if(differencePromotion <= maximumPromotionPointAllowUseInEachBill){
                                differencePromotion
                            }else{
...
//            binding.footer.tvPercentVat.text = String.format("(%s%s", data.vat.toString(), "%):")
            ...
            binding.footer.footerPayment.tvNotePromotionPoint.text = String.format(
                "%s %s%s %s",
                requireActivity().resources.getString(R.string.using),
                restaurantCard.maximum_percent_order_amount_to_promotion_point_allow_use_in_each_bill,
                requireActivity().resources.getString(R.string.percent),
                requireActivity().resources.getString(R.string.note_promotion_point)
            )
            // alo point
            ...
            if (data.maximum_alo_point_allow_use_in_each_bill!! > 0) {
                binding.footer.footerPayment.tvNoteAloPoint.text = String.format(
                    "%s %s%s %s %s %s",
                    requireActivity().resources.getString(R.string.note_alo_point_1),
                    restaurantCard.maximum_percent_order_amount_to_alo_point_allow_use_in_each_bill,
                    requireActivity().resources.getString(R.string.percent),
                    requireActivity().resources.getString(R.string.note_alo_point_2),
                ...
                binding.footer.footerPayment.tvNoteAloPoint.text = String.format(
                    ...
                    requireActivity().resources.getString(R.string.note_not_yet_supported_use_point_value)
                )
            }
    ...
    private fun setVisibilityButton(billTable: BillTable) {
        ...
        if (promotionPoint > 0 && billTable.maximum_promotion_point_allow_use_in_each_bill!! > 0) {
            if (billTable.membership_promotion_point_used!! > 0) {
                ...
                if ((billTable.membership_point_used!! + billTable.membership_accumulate_point_used!!) < billTable.maximum_point_allow_use_in_each_bill ?: 0){
                    binding.footer.footerPayment.lnUsePromotionPoint.show()
            ...
            .subscribe(object : Observer<BillTableResponse> {
                ...
                override fun onNext(response: BillTableResponse) {
                    if (response.status == AppConfig.SUCCESS_CODE) {
                        ...
                        mainActivity!!,
                        mainActivity?.getString(R.string.api_error),
                        Toast.LENGTH_LONG
                    ).show()
                    mainActivity?.setLoading(false)
                }
    ...
    override fun onGiftFood(food: OrderDetailBill) {
        ...
        tvNoteFoodBill.text = mainActivity?.getString(R.string.note_gift_bill).toString()
