...
import vn.techres.line.databinding.ItemLinkChatLeftBinding
...
class LinkMessageLeftHolder(private val binding : ItemLinkChatLeftBinding) : RecyclerView.ViewHolder(binding.root) {
    fun bind(context : Context, user : User, configNodeJs : ConfigNodeJs, dataSource: ArrayList<MessagesByGroup>, position : Int, chatGroupHandler: ChatGroupHandler?,
             revokeMessageHandler: RevokeMessageHandler?, chooseNameUserHandler: ChooseNameUserHandler?){
...
//        binding.tvTimeLink.text = TimeFormatHelper.getDateFromStringDay(message.created_at)
        ...
        if(message.message_share != null && !StringUtils.isNullOrEmpty(message.message_share?.random_key)){
            ...
            Utils.getMediaGlide(binding.link.imgLinkPreview, message.message_share?.message_link?.media_thumb)
        }else{
        ...
        ChatUtils.setViewTimeLeft(binding.reaction.lnReaction, binding.lnLink)
