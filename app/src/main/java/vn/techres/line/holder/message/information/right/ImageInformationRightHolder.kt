...
import vn.techres.line.databinding.ItemImageInfomationRightBinding
...
class ImageInformationRightHolder(private val binding : ItemImageInfomationRightBinding) : RecyclerView.ViewHolder(binding.root) {
                ...
                model?.files?.forEachIndexed { index, fileNodeJs ->
                    if (fileNodeJs.link_original.equals(it.link_original)) {
                        ...
                        TasksManager().getImpl()
                            .updateViewHolder(id, itemView.tag as ImageInformationRightHolder)
                        task.start()
...
//            }
        ...
        adapter?.setDataSource(message.files, 1, message)
...
//        }
    ...
    private val taskDownloadListener = object : FileDownloadingListener() {
        ...
        private fun checkCurrentHolder(task: BaseDownloadTask): ImageInformationRightHolder? {
            return if (task.tag is ImageInformationRightHolder) {
                val tag = task.tag as ImageInformationRightHolder
                if (tag.id != task.id) {
                    return null
                }
                tag
