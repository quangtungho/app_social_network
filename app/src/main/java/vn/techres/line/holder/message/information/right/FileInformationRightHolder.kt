...
import vn.techres.line.databinding.ItemFileInfomationRightBinding
...
class FileInformationRightHolder(private val binding: ItemFileInfomationRightBinding) :
...
//        binding.reaction.imgReactionPress.visibility = View.GONE
        ...
        binding.btnDownloadFile.setOnClickListener {
            ...
            id = task.id
            TasksManager().getImpl().updateViewHolder(id, it.tag as FileInformationRightHolder)
    ...
    private val taskDownloadListener = object : FileDownloadingListener() {
        ...
        private fun checkCurrentHolder(task: BaseDownloadTask): FileInformationRightHolder? {
            return if (task.tag is FileInformationRightHolder) {
                val tag = task.tag as FileInformationRightHolder
                if (tag.id != task.id) {
                    return null
                }
                tag
