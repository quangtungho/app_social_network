...
import vn.techres.line.databinding.ItemAudioInformationRightBinding
...
class AudioInformationRightHolder(private val binding : ItemAudioInformationRightBinding) : RecyclerView.ViewHolder(binding.root) {
            ...
            if (!File(path).exists()) {
                ...
                model?.files?.forEachIndexed { _, fileNodeJs ->
                    if (fileNodeJs.link_original.equals(it.link_original)) {
                        val url =
                            String.format("%s%s", nodeJs.api_ads, it.link_original)
                        ...
                        TasksManager().getImpl()
                            .updateViewHolder(id, itemView.tag as AudioInformationRightHolder)
                        task.start()
            ...
            if (!mediaPlayer!!.isPlaying) {
                ...
                run = Runnable {
                    ...
                    if (mediaPlayer.currentPosition != 0) {
                        binding.tvTimePlayAudio.text = TimeFormatHelper.calculateDuration(
...
//        }
    ...
    private val taskDownloadListener = object : FileDownloadingListener() {
        ...
        private fun checkCurrentHolder(task: BaseDownloadTask): AudioInformationRightHolder? {
            return if (task.tag is AudioInformationRightHolder) {
                val tag = task.tag as AudioInformationRightHolder
                if (tag.id != task.id) {
                    return null
                }
                tag
