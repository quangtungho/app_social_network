...
import vn.techres.line.databinding.ItemFileChatRightBinding
...
class FileMessageRightHolder(val binding: ItemFileChatRightBinding) :
...
//        binding.tvTimeFile.text = TimeFormatHelper.getDateFromStringDay(message.created_at)
        ...
        ChatUtils.setViewTimeRight(binding.reaction.lnReaction, binding.rlFileContainer)
        ...
        binding.btnDownloadFile.setOnClickListener {
            ...
            id = task.id
            TasksManager().getImpl().updateViewHolder(id, it.tag as FileMessageRightHolder)
            task.start()
        }
    ...
    private val taskDownloadListener = object : FileDownloadingListener() {
        ...
        private fun checkCurrentHolder(task: BaseDownloadTask): FileMessageRightHolder? {
            return if (task.tag is FileMessageRightHolder) {
                val tag = task.tag as FileMessageRightHolder
                if (tag.id != task.id) {
                    return null
                }
                tag
