...
import vn.techres.line.databinding.ItemVideoInfomationRightBinding
...
class VideoInformationRightHolder(private val binding: ItemVideoInfomationRightBinding) :
                ...
                model?.files?.forEachIndexed { _, fileNodeJs ->
                    if (fileNodeJs.link_original.equals(it.link_original)) {
                        ...
                        TasksManager().getImpl()
                            .updateViewHolder(id, itemView.tag as VideoInformationRightHolder)
                        task.start()
...
//        binding.reaction.imgReactionPress.visibility = View.GONE
    ...
    private val taskDownloadListener = object : FileDownloadingListener() {
        ...
        private fun checkCurrentHolder(task: BaseDownloadTask): VideoInformationRightHolder? {
            return if (task.tag is VideoInformationRightHolder) {
                val tag = task.tag as VideoInformationRightHolder
                if (tag.id != task.id) {
                    return null
                }
                tag
