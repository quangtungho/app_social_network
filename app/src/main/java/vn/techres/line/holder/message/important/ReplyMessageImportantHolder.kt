 ...
 * @Date: 21/09/2022
 */
class ReplyMessageImportantHolder(private val binding : ItemTextMessageImportantBinding) : RecyclerView.ViewHolder(binding.root){
    fun bind(configNodeJs: ConfigNodeJs, message : MessagesByGroup, messageImportantHandler: MessageImportantHandler?, chooseNameUserHandler: ChooseNameUserHandler?){
        binding.imgAvatar.loadImage(String.format("%s%s", configNodeJs.api_ads, message.message_reply!!.sender!!.avatar?.medium), R.drawable.ic_user_placeholder_circle, true)
        binding.tvName.text = message.message_reply!!.sender!!.full_name
        ...
        binding.imgAvatar.setOnClickListener {
            messageImportantHandler?.onChooseSender(message.message_reply!!.sender!!)
        }
