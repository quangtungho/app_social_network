...
import vn.techres.line.databinding.ItemReplyChatLeftBinding
...
class ReplyMessageLeftHolder(val binding: ItemReplyChatLeftBinding) :
...
//        binding.tvTimeReply.text = TimeFormatHelper.getDateFromStringDay(message.created_at)
        ...
        binding.tvNameReply.text = message.sender.full_name
        ...
        if (position >= 0 && (position + 1) < dataSource.size) {
            if (dataSource[position].sender.member_id == dataSource[position + 1].sender.member_id && (dataSource[position + 1].message_type.equals(
                    ...
                    binding.tvNameReply.hide()
                    binding.cvReply.visibility = View.INVISIBLE
                } else {
                binding.tvNameReply.show()
                binding.cvReply.show()
            }
        }
        if (message.today == 1) {
            ...
            if (message.message_reply?.status == 1) {
                when (message.message_reply?.message_type) {
                    ...
                    TechResEnumChat.TYPE_STICKER.toString() -> {
                        ...
                        getGlide(
                            binding.reply.imgReply,
                            message.message_reply?.files?.get(0)?.link_original,
                            configNodeJs
                        )
                    }
                ...
                binding.reply.tvReplyContent.text =
                    context.resources.getString(R.string.revoke_message)
                binding.reply.rlReplyThumbContainer.hide()
            }
        ...
        ChatUtils.setViewTimeLeft(binding.reaction.lnReaction, binding.lnReply, 4f)
