...
class BubbleChatAdapter(var context: Context) : RecyclerView.Adapter<RecyclerView.ViewHolder>(),
    ...
    private var videoMessageHandler: VideoMessageHandler? = null
    ...
    fun setVideoMessageHandler(videoMessageHandler: VideoMessageHandler){
...
//                    }
    ...
    private fun updateNonPlayingView(holder: RecyclerView.ViewHolder, message: MessagesByGroup? = null) {
        when (holder) {
            is AudioMessageRightHolder -> {
                ...
                message?.let {
                    holder.binding.tvTimePlayAudio.text =
                        if (it.message_share != null && !StringUtils.isNullOrEmpty(it.message_share?._id)) {
                            TimeFormatHelper.calculateDuration(it.message_share?.files?.get(0)?.time ?: 0)
                        } else {
    ...
    companion object {
        ...
        private const val TYPE_SHARE = 31
