...
import vn.techres.line.helper.rtc_custom.rtc_client.AppRTCClient;
...
public class PeerConnectionClient {
  ...
  private static final String RTP_DATA_CHANNELS = "RtpDataChannels";
  ...
  private boolean renderUserVideo = true;
...
//    ((ProxyVideoSink) remoteSinks.get(0)).setTarget(surfaceViewRenderer);
//  }
  ...
  public interface PeerConnectionEvents {
    ...
    /**
     * Callback bytes data message receiver
     * */
    void onSoundWave(MediaStream stream);
  }
  ...
  private void createMediaConstraintsInternal() {
    ...
    if (peerConnectionParameters.noAudioProcessing) {
      ...
      audioConstraints.optional.add(new MediaConstraints.KeyValuePair(RTP_DATA_CHANNELS, "false"));
      audioConstraints.optional.add(new MediaConstraints.KeyValuePair(DTLS_SRTP_KEY_AGREEMENT_CONSTRAINT, "true"));
    }
  ...
  public void setVideoEnabledUser(final boolean enable, final boolean isSwappedFeeds){
    executor.execute(() -> {
      renderUserVideo = enable;
      if(isSwappedFeeds){
        if (localVideoTrack != null) {
          localVideoTrack.setEnabled(renderUserVideo);
        }
      }else{
        if (remoteVideoTrack != null) {
          remoteVideoTrack.setEnabled(renderUserVideo);
        }
      }
    });
  }
...
//      WriteLog.INSTANCE.d("getFieldTrials", "Enable FlexFEC field trial.");
    }
    fieldTrials += VIDEO_VP8_INTEL_HW_ENCODER_FIELDTRIAL;
    if (peerConnectionParameters.disableWebRtcAGCAndHPF) {
      fieldTrials += DISABLE_WEBRTC_AGC_FIELDTRIAL;
//      Timber.d("Disable WebRTC AGC field trial.");
      WriteLog.INSTANCE.d("getFieldTrials", "Disable WebRTC AGC field trial.");
    }
    return fieldTrials;
  }
  ...
  private static String setStartBitrate(
    ...
    Timber.d("Found " + codec + " rtpmap " + codecRtpMap + " at " + lines[rtpmapLineIndex]);
  ...
  private class PCObserver implements PeerConnection.Observer {
    ...
    public void onIceCandidate(final IceCandidate candidate) {
      executor.execute(() ->{
        WriteLog.INSTANCE.d("CreateIceCandidate...", new Gson().toJson(candidate) + "");
    ...
    public void onSignalingChange(PeerConnection.SignalingState newState) {
      WriteLog.INSTANCE.d("SignalingState: %s", newState.name());
    }
    ...
    public void onDataChannel(final DataChannel dc) {
      ...
      WriteLog.INSTANCE.d("onDataChannel", "New Data channel" + dc.label() + dataChannelEnabled);
      ...
      dc.registerObserver(new DataChannel.Observer() {
        ...
        public void onBufferedAmountChange(long previousAmount) {
          WriteLog.INSTANCE.d("onBufferedAmountChange", "Data channel buffered amount changed: " + dc.label() + ": " + dc.state());
        }

        @Override
        public void onStateChange() {
          WriteLog.INSTANCE.d("onStateChange", "Data channel state changed: " + dc.label() + ": " + dc.state());
        }

        @Override
        public void onMessage(final DataChannel.Buffer buffer) {
          if (buffer.binary) {
            WriteLog.INSTANCE.d("onMessageReceived", "Received binary msg over %s" + dc);
            return;
          }
          ...
          WriteLog.INSTANCE.d("onMessage", "Got msg: " + strData + " over " + dc);
        }
      });
    }
  ...
  private class SDPObserver implements SdpObserver {
    ...
    public void onCreateSuccess(final SessionDescription origSdp) {
      ...
      executor.execute(() -> {
        if (peerConnection != null && !isError) {
          WriteLog.INSTANCE.d("Set local SDP from", sdp.type.canonicalForm());
