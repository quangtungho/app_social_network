    ...
    private var progress = 0f
        set(value) {
            ...
            ViewCompat.setElevation(
                v_circle, if (progress > 0.7f) dipf(
                    context,
                    progress * 4f
                ) else 0f
            )
            ...
            v_circle.x = (1f - progress) * (if (isFromLeft) -m else m) + ((measuredWidth - dip(
                context,
                48
            )) / 2f)
            v_circle.y = (1f - progress) * measuredHeight + dip(
                context,
                6
            )
        }
