...
public class AXCategoryAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        layout.addView(icon, new AXEmojiLayout.LayoutParams(Utils.dpToPx(viewGroup.getContext(), 7),
                Utils.dpToPx(viewGroup.getContext(), 7), iconSize, iconSize));
        layout.setLayoutParams(new ViewGroup.LayoutParams(Utils.dpToPx(viewGroup.getContext(), 38),
                Utils.dpToPx(viewGroup.getContext(), 38)));
        ProgressBar pbLoading = new ProgressBar(viewGroup.getContext());
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(iconSize,iconSize);
        params.gravity = Gravity.CENTER;
        layout.addView(pbLoading, params);

        int dp8 = Utils.dpToPx(viewGroup.getContext(), 8);
        pbLoading.setPadding(dp8, dp8, dp8, dp8);

        View selection = new View(viewGroup.getContext());
        layout.addView(selection, new AXEmojiLayout.LayoutParams(
                0, Utils.dpToPx(viewGroup.getContext(), 36), Utils.dpToPx(viewGroup.getContext(), 38),
                Utils.dpToPx(viewGroup.getContext(), 2)));
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, @SuppressLint("RecyclerView") final int i) {
        ...
        View pbLoading = layout.getChildAt(1);
        View selection = (View) layout.getChildAt(2);
        ...
        if (recent && i == 0) {
            ...
            int i2 = i;
            if (recent) i2--;
            provider.getLoader().onLoadStickerCategory(((ImageView) icon), ((ProgressBar) pbLoading), provider.getCategories()[i2], selected);
        }
