...
public class AXEmojiPopupLayout extends FrameLayout implements AXPopupInterface {
    ...
    public void initPopupView(AXEmojiBase content, final AXEmojiBase contentOld) {
        if (keyboard == null) {
            keyboard = new View(getContext());
            this.addView(keyboard,new FrameLayout.LayoutParams(-1,0));
        }
        content.setPopupInterface(this);
        popupView = new AXEmojiPopupView(this, content, contentOld);
    ...
    public void initPopupView(AXEmojiBase content, final AXEmojiBase contentOld, final AXEmojiBase contentMedia) {
        if (keyboard == null) {
            keyboard = new View(getContext());
            this.addView(keyboard,new FrameLayout.LayoutParams(-1,0));
        }
        content.setPopupInterface(this);
        popupView = new AXEmojiPopupView(this, content, contentOld, contentMedia);
        popupView.setFocusableInTouchMode(true);
        popupView.setFocusable(true);
        popupView.requestFocus();
        initKeyboardHeightProvider();
    ...
    public void setView(Integer position){
        if (popupView != null){
            popupView.setView(position);
        }
    }
    public Integer isCheckView(){
        if(popupView != null){
            return popupView.isCheckView();
        }else{
            return 0;
        }
    }
