...
public class AXEmojiPopupView extends FrameLayout implements AXPopupInterface {
    ...
    AXEmojiBase content;
    AXEmojiBase contentOld;
    AXEmojiBase contentMedia;
    ...
    public AXEmojiPopupView(ViewGroup view, final AXEmojiBase content, final AXEmojiBase contentOld) {
        ...
        AXEmojiBase base = new AXEmojiBase(content.getContext());
        this.addView(base, new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT,
                FrameLayout.LayoutParams.MATCH_PARENT));
        base.addView(content);
        base.addView(contentOld);
        ...
        contentOld.setBackgroundColor(AXEmojiManager.getEmojiViewTheme().getBackgroundColor());
        this.invalidate();
    }

    public AXEmojiPopupView(ViewGroup view, final AXEmojiBase content, final AXEmojiBase contentOld, final AXEmojiBase contentMedia) {
        super(content.getContext());
        Utils.forceLTR(view);
        popupWindowHeight = Utils.getKeyboardHeight(content.getContext(), 0);
        lp = new FrameLayout.LayoutParams(-1, popupWindowHeight);
        ...
        content.setBackgroundColor(AXEmojiManager.getEmojiViewTheme().getBackgroundColor());
        contentOld.setBackgroundColor(AXEmojiManager.getEmojiViewTheme().getBackgroundColor());
        contentMedia.setBackgroundColor(AXEmojiManager.getEmojiViewTheme().getBackgroundColor());
        this.invalidate();
    }
    ...
    public void setView(Integer position){
        switch (position){
            case 1:{
                content.setVisibility(VISIBLE);
                contentOld.setVisibility(GONE);
                if(contentMedia != null){
                    contentMedia.setVisibility(GONE);
                }
                break;
            }
            case 2:{
                content.setVisibility(GONE);
                contentOld.setVisibility(VISIBLE);
                if(contentMedia != null){
                    contentMedia.setVisibility(GONE);
                }
                break;
            ...
            case 3:{
                contentMedia.setVisibility(VISIBLE);
                content.setVisibility(GONE);
                contentOld.setVisibility(GONE);
                break;
            }
    ...
    public Integer isCheckView(){
        if(contentOld.getVisibility() == VISIBLE){
            return 2;
        }
        ...
        if(contentMedia != null){
            if(contentMedia.getVisibility() == VISIBLE){
                return 3;
            }
        }
        return 0;
