...
public class DefaultChatHeadManager<T extends Serializable> implements ChatHeadCloseButton.CloseButtonListener, ChatHeadManager<T> {
    ...
    @Override
    public void onJoinBubble(ChatHead<T> chatHead) {
        if(listener != null){
            listener.onJoinBubble(chatHead.getKey());
        }
    }

    @Override
    public void onChangeBubble(ChatHead<T> oldChatHead, ChatHead<T> newChatHead) {
        if (listener != null){
            listener.onChangeBubble(oldChatHead.getKey(), newChatHead.getKey());
        }
    }

    @Override
    public Boolean onBackButton(ChatHead<T> chatHead) {
        if(listener != null){
            return listener.onBackPress(chatHead.getKey());
        }else{
            return null;
        }
    }
    ...
    public ChatHead<T> addChatHead(T key, boolean isSticky, boolean animated) {
        ...
        if (chatHead == null) {
            ...
            ViewGroup.LayoutParams layoutParams = chatHeadContainer.createLayoutParams(getConfig().getHeadWidth(),
                    getConfig().getHeadHeight(), Gravity.START | Gravity.TOP, 0);
...
//            if (((NotifyBubbleChat) chatHead.getKey()).getValue().equals(((NotifyBubbleChat)key).getValue()))
                return chatHead;
        }
        return null;
    }
    ...
    private void init(Context context, ChatHeadConfig chatHeadDefaultConfig) {
        ...
        ViewGroup.LayoutParams shadowLayoutParams = chatHeadContainer.createLayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.MATCH_PARENT, Gravity.BOTTOM, 0);
    ...
    private void setupOverlay(Context context) {
        ...
        ViewGroup.LayoutParams layoutParams = getChatHeadContainer().createLayoutParams(WindowManager.LayoutParams.MATCH_PARENT,
                WindowManager.LayoutParams.MATCH_PARENT, Gravity.NO_GRAVITY, 0);
