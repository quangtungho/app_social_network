...
public interface ChatHeadManager<T extends Serializable> {
     ...
     * Only when  &&  returns a positive value will arrangements start working
     *
     * @param height
     * @param width
     */
    ...
    Boolean onBackButton(ChatHead<T> chatHead);

    void onJoinBubble(ChatHead<T> chatHead);

    void onChangeBubble(ChatHead<T> oldChatHead, ChatHead<T> newChatHead);
